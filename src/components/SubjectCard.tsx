  ...
  return (
    ...
    >
      <Link to={`/subjects/${subject.id}`} className="block">
        ...
        <h3 className="mb-3 text-2xl font-semibold tracking-tight text-gray-900">{subject.name}</h3>
        <p className="mb-6 text-base leading-relaxed text-gray-600">{subject.description}</p>
        
        <div className="flex items-center gap-6 text-sm font-medium text-gray-500">
          <div className="flex items-center gap-2">
            <NotebookPen size={18} className="text-mint-500" strokeWidth={1.5} />
            <span>{subject.notesCount} Notes</span>
          </div>
          <div className="flex items-center gap-2">
            <FileText size={18} className="text-mint-500" strokeWidth={1.5} />
            <span>{subject.papersCount} Papers</span>
          </div>
          <div className="flex items-center gap-2">
            <Book size={18} className="text-mint-500" strokeWidth={1.5} />
